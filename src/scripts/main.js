...
function moveRange (elem) {
    ...
    $(document).on('click', ".cards-img__button-right",function(){
                var carusel = $(this).parents('.cards-img__carousel');
                right_carusel(carusel);
                return false;
            });


            $(document).on('click',".cards-img__button-left",function(){
                var carusel = $(this).parents('.cards-img__carousel');
                left_carusel(carusel);
                return false;
            });
            function left_carusel(carusel){
                var block_width = $(carusel).find('.cards-img__carousel-block').outerWidth();
                $(carusel).find(".cards-img__carousel-items .cards-img__carousel-block").eq(-1).clone().prependTo($(carusel).find(".cards-img__carousel-items"));
                $(carusel).find(".cards-img__carousel-items").css({"left":"-"+block_width+"px"});
                $(carusel).find(".cards-img__carousel-items .cards-img__carousel-block").eq(-1).remove();
                $(carusel).find(".cards-img__carousel-items").animate({left: "0px"}, 200);

            }
            function right_carusel(carusel){
                var block_width = $(carusel).find('.cards-img__carousel-block').outerWidth();
                $(carusel).find(".cards-img__carousel-items").animate({left: "-"+ block_width +"px"}, 200, function(){
                    $(carusel).find(".cards-img__carousel-items .cards-img__carousel-block").eq(0).clone().appendTo($(carusel).find(".cards-img__carousel-items"));
                    $(carusel).find(".cards-img__carousel-items .cards-img__carousel-block").eq(0).remove();
                    $(carusel).find(".cards-img__carousel-items").css({"left":"0px"});
                });
            }
